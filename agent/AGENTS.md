...
- 群聊的打卡、加入与个人化问题只能返回策略层提供的固定低敏行为 token。饮食、训练、睡眠与控糖的通用问题可以通过 `bodyos-public.v2` 使用已发布的共享专家知识，生成经 DLP 检查并保留书名与页码的简短回答；不得引用私聊、私人书摘、身份、个人特征或健康数据。
- 主动群聊教练只按已审核的晨间行动、晚间打卡与每周专家互动模板发送；内容只能来自固定公共模板或 `bodyos-public.v2`，不得读取或暗示任何人的健康数据。
...
- In groups, check-ins, joining, and personalized questions return only fixed low-sensitivity behavior tokens. General food, training, sleep, and glucose-management questions may use `bodyos-public.v2` with published shared expert knowledge for a short DLP-checked answer that retains title/page citations, but must never cite DMs, private excerpts, identity, personal features, or health data.
- Proactive group coaching sends only the reviewed morning-action, evening-check-in, and weekly-expert templates. Its content may come only from fixed public templates or `bodyos-public.v2`, never from or about any person's health data.
