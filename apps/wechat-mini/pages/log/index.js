...
Page(base({data:{energyLabels:['很低','偏低','适中','不错','充足'],energy:3,stress:2,feelingIndex:1,feelings:['充沛','正常','有点累','很累','不适'],note:'',sleepIndex:0,trainingIndex:0,sourceIndex:0,sleepFeelings:['暂不记录','醒后清爽','一般','醒后疲惫'],trainingFeelings:['暂不记录','完成','偏累','恢复良好'],stressSources:['暂不记录','工作','学习','人际','其他']},
 onLoad(){const draft=wx.getStorageSync('fitcrew.draft');if(draft)this.setData(draft);},
 choose(e){this.edit({currentTarget:e.currentTarget,detail:{value:e.currentTarget.dataset.value}});},
 edit(e){this.syncBoundary();if(this.data.busy)return;const field=e.currentTarget.dataset.field,value=field==='note'?e.detail.value:Number(e.detail.value);this.setData({[field]:value,notice:''});wx.setStorageSync('fitcrew.draft',this.draft());},
 ...
 async remove(e){this.syncBoundary();const epoch=lifecycle.epoch(wx);if(this.data.busy||!await confirm('删除这条记录？','这会永久删除记录，并使相关实验评估失效。'))return;if(!lifecycle.current(wx,epoch)||this.data.busy)return;this.invalidateRefresh();this.setData({busy:true,loading:false,error:''});try{const receipt=await getApp().api.request('/v3/logs/'+e.currentTarget.dataset.id,'DELETE');if(!lifecycle.current(wx,epoch))return;if(!receipt.deleted||!receipt.receipt_id)throw new Error('服务未确认删除完成，请重试。');this.setData({receipt:receipt.receipt_id});await this.refresh();}catch(e){if(lifecycle.current(wx,epoch))this.setData({error:e.message});}finally{if(lifecycle.current(wx,epoch))this.setData({busy:false});}}
}));
