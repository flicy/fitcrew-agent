...
function base(extra={}) {
 const initial={loading:false,busy:false,error:'',state:null,experiments:[],logs:[],trendPoints:[],trendDays:30,observedDays:0,receipt:'',...extra.data};
 return {
  ...
  async refresh(){
   ...
   catch(e){if(session.current(wx,epoch))this.setData({error:e.message,state:null,logs:[],experiments:[],trendPoints:[],observedDays:0});}
  ...
  updateTrends(){const points=this.data.state&&this.data.state.trends?this.data.state.trends.points.slice(-this.data.trendDays):[];this.setData({trendPoints:points.map(p=>({...p,barHeight:p.energy===null?0:p.energy*20})),observedDays:points.filter(p=>p.count>0).length});},
