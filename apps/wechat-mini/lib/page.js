...
const {textScale,healthPoints,healthReadiness}=require('./presentation');
...
function base(extra={}) {
 const initial={healthReadiness:null,healthMetric:'sleep',healthPoints:[],healthObservedDays:0,selectedHealthPoint:null,textScale:1,notice:'',loading:false,busy:false,error:'',state:null,experiments:[],logs:[],trendPoints:[],trendDays:30,selectedTrend:null,observedDays:0,receipt:'',...extra.data};
 return {
  invalidateRefresh(){this._refreshRevision=(this._refreshRevision||0)+1;},
  resetPrivate(){this.invalidateRefresh();this.setData({...JSON.parse(JSON.stringify(initial)),signedIn:!!wx.getStorageSync('fitcrew.session')});this._epoch=session.epoch(wx);},
  syncBoundary(){session.watch(wx,this);session.active(wx);if(this._epoch!==session.epoch(wx))this.resetPrivate();},
  onUnload(){this.invalidateRefresh();session.unwatch(wx,this);},
  async refresh(){
   this.syncBoundary();this.invalidateRefresh();const revision=this._refreshRevision,epoch=session.epoch(wx),current=()=>session.current(wx,epoch)&&this._refreshRevision===revision;this.setData({loading:true,error:'',notice:''});
   try{const state=await getApp().api.request('/v3/state');if(!current())return;this.setData({state,logs:[...state.logs].reverse(),experiments:state.experiments.map(x=>({...x,statusLabel:labels[x.status]||x.status,sourceLabel:x.source==='ai_selected'?'AI 选择 · 受约束行动':x.source==='rule_based'?'规则建议':'来源待确认',resultText:x.result?x.result.summary:''}))});this.updateTrends();}
   catch(e){if(current())this.setData({error:e.message,state:null,healthReadiness:null,logs:[],experiments:[],trendPoints:[],selectedTrend:null,observedDays:0,healthPoints:[],selectedHealthPoint:null,healthObservedDays:0});}
   finally{if(current())this.setData({loading:false});}
  },
  async write(key,path,body,method='POST'){
   this.syncBoundary();const epoch=session.epoch(wx);if(this.data.busy)return false;this.invalidateRefresh();this.setData({busy:true,loading:false,error:'',notice:''});
  ...
  updateTrends(){const points=this.data.state&&this.data.state.trends?this.data.state.trends.points.slice(-this.data.trendDays):[];this.setData({healthReadiness:healthReadiness(this.data.state),selectedTrend:null,trendPoints:points.map(p=>({...p,barHeight:p.energy===null?0:p.energy*20})),observedDays:points.filter(p=>p.count>0).length});this.updateHealthTrends();},
