...
- `uv run pytest -q`：308 项通过；唯一警告来自既有 Starlette/httpx 测试兼容性弃用提示。
...
- `uv run pytest -q`: 308 passed; the only warning is the existing Starlette/httpx test-compatibility deprecation.
