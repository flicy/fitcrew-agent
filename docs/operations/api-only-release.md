...
独立诊断工具需要 Python 3.11 或更新版本，与项目最低版本一致；旧解释器会在执行任何检查命令前退出并提示版本要求。
...
The standalone diagnostic requires Python 3.11 or newer, matching the project minimum. Older interpreters exit with a version message before any diagnostic command runs.
