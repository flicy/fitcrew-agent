(async()=>{
  ...
  const makeFixture=Function(fixtureScript+';return makeFixture;')();
  ...
  function mount(frame,bundle){
    const route=controls.page.value,scenario=controls.scenario.value,width=Number(controls.width.value),scale=Number(controls.scale.value),storage={};
    let state=makeFixture(scenario,controls.step.value),page,schedule=false;
    const fmt={day:v=>v?v.slice(5,10).replace('-','/'):'',time:v=>v?v.replace('T',' ').slice(0,16):'尚无记录',mean:v=>v==null?'—':Math.round(v*10)/10};
    ...
    if(scenario!=='restricted')storage['fitcrew.session']={device_token:'synthetic-local-preview',created_at:Date.now()};
    ...
    async function request(path,method='GET',body){
      ...
      if(path==='/v3/capabilities')return {ai_available:false,ai_consent_granted:false,ai_notice:'AI 需要单独授权。仅使用受限的聚合信息，不读取笔记或原始健康样本。',ai_provider:'',ai_notice_version:''};
      if(path==='/v3/state')return structuredClone(state);
      ...
      if(path==='/v3/experiments/propose'){state.experiments.unshift({...state.experiments[0],id:'preview-proposal',status:'proposed',result:null});return state.experiments[0];}
      if(path.endsWith('/feedback')){const item=state.experiments.find(e=>e.id===path.split('/')[3]);item.user_feedback={assessment:body.assessment};return item;}
